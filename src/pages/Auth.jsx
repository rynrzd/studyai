...
export default function Auth() {
  ...
  const handleSubmit = () => {
    ...
    try {
      const user = isLogin
        ? login(form.email, form.pwd)
        : register({ name: form.name, email: form.email, password: form.pwd, classe: form.classe, matiere: form.matiere || undefined, noteActuelle: form.note || undefined });
      ctxLogin(user);
    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 20, background: "var(--bg)" }}>
      <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 28, padding: "40px 34px", width: "100%", maxWidth: 430, boxShadow: "0 24px 80px rgba(0,0,0,0.1)" }}>
        ...
        <p style={{ color: "var(--text-muted)", fontSize: 14, marginBottom: 26 }}>
          {isLogin ? "Connexion à ton compte StudyAI" : "Gratuit · Change de plan à tout moment"}
        </p>
        ...
        <div style={{ textAlign: "center", fontSize: 14, color: "var(--text-muted)", marginTop: 18 }}>
          ...
          <span onClick={() => { setAuthMode(isLogin ? "signup" : "login"); setErr(""); }}
